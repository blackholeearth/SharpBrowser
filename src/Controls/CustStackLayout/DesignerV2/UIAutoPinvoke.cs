...
namespace SharpBrowser
{
    public static class UIAutoPinvoke 
    {
        ...
        /// <summary>
        ///  if you wan to optimize  -- call this textComparison multiple times..  <para/>
        ///  var prochandles = Get_Process_EnumerateProcessWindowHandles(string processName) ; <para/>
        ///  var ptr = GetWindowHandle_byText(prochandles , x=> x.Contains(child_WindowCaption), Cmp.Text);
        /// </summary>
        public static IntPtr GetWindowHandle_byText(string processName, Func<string, bool> compareTitle, Cmp cmp)
        {
            ...
            {
                LayoutLogger.Log("GetWindowHandle_byText-> forloop -> handle:"+ handle);
        ...
        public static IEnumerable<nint> Get_Process_EnumerateProcessWindowHandles(string processName)
        {
            //var processName = "explorer";
            var proc = Process.GetProcessesByName(processName).FirstOrDefault();

            return Get_Process_EnumerateProcessWindowHandles(proc);
        }
        public static IEnumerable<nint> Get_Process_EnumerateProcessWindowHandles(Process process)
        {
            //var processName = "explorer";
            if (process == null)
                return null;

            return EnumerateProcessWindowHandles(process.Id);
        }
        ...
        ///  var prochandles = Get_Process_EnumerateProcessWindowHandles(string processName) ; <para/>
        ///  var ptr = GetWindowHandle_byText(prochandles , x=> x.Contains(child_WindowCaption), Cmp.Text);
        public static IntPtr GetWindowHandle_byText(IEnumerable<nint> ProcessWindowHandles, 
            Func<string, bool> compareTitle, Cmp cmp)
        {

            if (ProcessWindowHandles == null)
                return IntPtr.Zero;
